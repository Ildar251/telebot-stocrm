...
const { MONGODB_URI, STO_BASE_URL, STO_SID, CORS_ORIGIN, PORT, SMSC_LOGIN, SMSC_PASSWORD, BOT_TOKEN, } = process.env;
// ===== Mongo модель =====
...
async function getLinkOr403(req, res) {
...
async function ensureOfferBelongsTo(link, offerId) {
    const payload = { SID: STO_SID, FILTER: { OFFER_ID: Number(offerId) } };
    const r = await fetch(`${STO_BASE_URL}/api/external/v1/offers/get_from_filter`, {
        method: 'POST',
...
async function fetchStoFile(guid) {
...
async function sendToTelegramDocument(params) {
    ...
    if (!BOT_TOKEN)
        throw new Error('BOT_TOKEN is required on server');
    ...
    if (!tgRes.ok || !tgJson?.ok) {
        throw new Error(`Telegram sendDocument failed: ${tgRes.status} ${JSON.stringify(tgJson).slice(0, 300)}`);
    }
    return tgJson;
}
// ====== SMSC отправка =====
async function sendSms(phone, text) {
    const url = `https://smsc.ru/sys/send.php?login=${encodeURIComponent(SMSC_LOGIN)}&psw=${encodeURIComponent(SMSC_PASSWORD)}&phones=${encodeURIComponent(phone)}&mes=${encodeURIComponent(text)}&fmt=3`;
...
async function main() {
    ...
    app.get('/api/file/:guid', async (req, res) => {
        try {
            ...
            if (!guid)
                return res.status(400).send('guid required');
            ...
            res.setHeader('Content-Disposition', r.headers.get('content-disposition') || `inline; filename="${guid}"`);
            const buffer = Buffer.from(await r.arrayBuffer());
            res.end(buffer);
        }
        catch (e) {
    ...
    app.post('/api/offers/:id/send-to-chat', async (req, res) => {
        ...
        if (!id)
            return res.status(400).json({ ok: false, message: 'Bad offer id' });
        ...
        try {
            ...
            if (guid) {
                ...
                const url = `${STO_BASE_URL}/api/external/v1/files/get_filtered_attaches?SID=${encodeURIComponent(STO_SID)}`;
                ...
                guids = list.map((x) => String(x.GUID)).filter(Boolean);
                for (const a of list) {
                    if (a?.GUID && a?.NAME)
    ...
    app.post('/api/auth/request', async (req, res) => {
        try {
            ...
            await LinkModel.updateOne({ telegram_id: Number(tg.id) }, {
                $set: {
                    phone: '+' + phone7,
                    sms_code: code,
                    sms_expire: expire,

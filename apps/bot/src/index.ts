...
async function main() {
	...
	async function sendAuthUI(chatId: number, url: string) {
		...
		// 2) Reply‑клавиатура (может быть «свёрнута» — иконка клавиатуры слева от ввода)
		await safeReply(
			chatId,
			'Если не видите кнопку — нажмите иконку клавиатуры слева от поля ввода.',
			{
				reply_markup: {
					keyboard: [[{ text: '🔐 Войти', web_app: { url } }]],
